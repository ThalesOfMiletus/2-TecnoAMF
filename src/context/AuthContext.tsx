...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  async function signIn({ username, password }: SignInCredentials) {
    try {
      const response = await api.post<AuthTokenPayload>(
        "/autenticacao/token/",
        {
          username,
          password,
      ...
      setCookie(undefined, "myapp.token", access, {
        maxAge: 60 * 60 * 24 * 30,
        path: "/",
      });
      setCookie(undefined, "myapp.refreshToken", refresh, {
        maxAge: 60 * 60 * 24 * 30,
        path: "/",
      });
      api.defaults.headers.common["Authorization"] = `Bearer ${access}`;
      const userResponse = await api.post<User>("/autenticacao/login/", {
        username,
        password,
      });
